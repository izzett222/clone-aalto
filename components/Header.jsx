...
function handleClick() {
  ...
  return (
    <>
      <header
        className={` bg-[#2d2d2d] fixed top-0 inset-x-0 z-30 border-b border-b-white px-4 md:pl-20 md:pr-14 flex  justify-between items-center h-[50px]  ${
          scrolled ? "md:h-[50px]" : "md:h-[110px]"
      ...
      >
        <Image
          ...
          className={`h-[33px] w-[37px] ${
            scrolled ? "md:h-10 md:w-10" : "md:w-[98px] md:h-[69px]"
          } duration-500`}
        />
        ...
        <div className="flex items-center gap-4 md:hidden">
          ...
          <button className="flex items-center gap-2 p-1 hover:bg-[#4A4A4A]" onClick={handleClick}>
             {open ? "close menu" : 'Menu'}
          </button>
        </div>
      </header>
      {open && <Menu scrolled={scrolled} />}
    </>
  );
}
